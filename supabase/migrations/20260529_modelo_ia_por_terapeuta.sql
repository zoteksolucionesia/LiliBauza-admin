...
ALTER TABLE configuracion_branding
  ADD COLUMN IF NOT EXISTS modelo_ia TEXT DEFAULT 'gemini-2.0-flash-lite';

COMMENT ON COLUMN configuracion_branding.modelo_ia IS 'Modelo de IA elegido por el terapeuta para interpretar tests (ej. gemini-2.0-flash-lite gratuito, gemini-1.5-pro de pago). La API key es central del sistema.';
